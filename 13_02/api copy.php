...
if (!empty($_GET['do'])) switch ($_GET['do']) {
  case 'login':
    ...
    case 'msgmdy':
        ...
        $db->query($sql);
        echo '<script>alert("修改成功, 如果有修改到名稱或mail請重新登入 "); location.href = "admin.php";</script>';
        break;
    case 'msgdel':
        $sql = "UPDATE `msg` SET `del`=1 WHERE id = ".$_GET['id']."";//sql語法錯誤 未加where語法
        $db->query($sql);
    ...
    case 'pkadd':
        ...
        $db->query($sql);
        echo '<script>alert("參賽成功！"); location.href = "index.php#pklist";</script>';
        break;
}